...
fn get_seeds_to_test_from_info(info: &str) -> Vec<SeedsToTest> {
    let mut seeds_to_test: Vec<SeedsToTest> = Vec::new();
    let mut split = info.split(" ");
    while let Some(from) = split.next() {
        let to = split.next().unwrap();
        seeds_to_test.push(SeedsToTest {
            from: from.parse::<usize>().unwrap(),
            length: to.parse::<usize>().unwrap(),
        });
    }
    return seeds_to_test;
}

fn aoc_day5_part2(file_content: &str) -> usize {
    let split = file_content.split_once("\n\r\n");
    let seeds = split.unwrap().0[split.unwrap().0.find(":").unwrap()+1..].trim();
    let seeds_to_test = get_seeds_to_test_from_info(seeds);
    ...
    for seed_range in seeds_to_test {
        for seed in seed_range.from..seed_range.from+seed_range.length {
...
fn main() {
    ...
    println!("{}", aoc_day5_part2(&file_content));
}